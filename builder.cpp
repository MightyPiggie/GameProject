...
void builder::build_underground(float height, bool force_underground_grass) {
    ...
    if(underground_type == 0 && force_underground_grass == false) {
        ...
        obstacle* tmp2 = new obstacle {window, sf::Vector2f{width_screen/4.f, height},  sf::Vector2f{width_screen/2.f, 59.0}, DEADLY, sprite_factory["water_sprite"]};
...
void builder::generate_obstacle_train(float height) {
    ...
    if(direction == 0) {
        obstacle_moving* tmp = new obstacle_moving {window, sf::Vector2f{width_screen/4*3.f+240, height}, sf::Vector2f{240.0, 59.0}, DEADLY, sprite_factory["train_left_sprite"], 100, direction , 4};
        sprite_builds.push_back(tmp);
    }
    else if(direction == 1) {
        obstacle_moving* tmp = new obstacle_moving {window, sf::Vector2f{width_screen/4.f-240, height}, sf::Vector2f{240.0, 59.0}, DEADLY, sprite_factory["train_right_sprite"], 100, direction , 4};
        sprite_builds.push_back(tmp);
    }
}
...
void builder::generate_obstacle_logs(float height) {
    ...
    if(direction == 0) {
        obstacle_moving* tmp = new obstacle_moving {window, sf::Vector2f{width_screen/4*3.f+60, height}, sf::Vector2f{119.0, 59.0}, FLOATING, sprite_factory[sprites_log[log_length]], 300, direction};
        sprite_builds.push_back(tmp);
    }
    else if(direction == 1) {
