...
//                                screenshot_gamescreen{}
{
    ...
    std::shared_ptr<buttons> pauze_button = std::make_shared<buttons>(window, 40,  vector2f_from_unsigned_ints(float(width) - 300.f,float(height)-200.f),  [&](){if(gameSettings.sound){click_sound.play();} gameSettings.started = !gameSettings.started; sf::sleep(sf::milliseconds(100));},"Pauze", sf::Color(163 , 235 , 177));
