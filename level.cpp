    ...
    if(game_setting.started){
        ...
        if(ticks % 60 == 0) {
            ...
            for(unsigned int index = 0; index < lines.size(); index++)
                if(lines[index]->get_position().y <= window.getSize().y+90) {
...
void level::build_line(float height, bool force_grass_line) {
    ...
    if(line_type == 0 && force_grass_line == false) {
        std::shared_ptr<line> create_rails_line = std::make_shared<line>(window, sf::Vector2f{window.getSize().x/4.f, height},  sf::Vector2f{window.getSize().x/2.f, 59.0}, RAILS, NON_OBSTACLE, "rails_sprite");
        lines.emplace_back(create_rails_line);
    }
    // Roads
    else if(line_type == 2 && force_grass_line == false) {  //todo WATER MUST BE CHANGED TO DEADLY
        std::shared_ptr<line> create_roads_line = std::make_shared<line>(window, sf::Vector2f{window.getSize().x/4.f, height},  sf::Vector2f{window.getSize().x/2.f, 59.0}, ROAD, NON_OBSTACLE, "roads_sprite");
        lines.emplace_back(create_roads_line);
    }
    // Water
    else if(line_type == 3 && force_grass_line == false) {
        std::shared_ptr<line> create_water_line = std::make_shared<line>(window, sf::Vector2f{window.getSize().x/4.f, height},  sf::Vector2f{window.getSize().x/2.f, 59.0}, WATER, DEADLY, "water_sprite");
        lines.emplace_back(create_water_line);
    }
    // Grass
    else if((line_type == 1 || 4) || force_grass_line == true) {
        // TODO No coins first 5
        std::shared_ptr<line> create_grass_line = std::make_shared<line>(window, sf::Vector2f{window.getSize().x/4.f, height},  sf::Vector2f{window.getSize().x/2.f, 59.0}, GRASS, NON_OBSTACLE, "grass_sprite");
