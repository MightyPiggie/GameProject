...
public:
    void loop(){
        ...
        player player1 {window , sf::Vector2f{ 960.0, 960.0 }, sf::Vector2f{ 0.0, 0.0 }, "player_sprite.png" , width , height};
        std::vector<drawable *> objects = {&left, &right, &game_window, &quit, &player1};
        while (window.isOpen()) {
            switch (state_t) {
                case GAME: {
