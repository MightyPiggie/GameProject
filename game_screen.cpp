...
void game_screen::run(){
    ...
    //lijst van objecten
    std::vector<drawable*> drawables = {&left,&game_window, &right, &quit_gamewindow, &back_to_menu_from_gamewindow,&player1, &test1, &display_coins};
    std::vector<game_object*> updateables = {&left,&game_window, &right, &quit_gamewindow, &back_to_menu_from_gamewindow,&player1, &test1, &display_coins, &start_game, &builder1};
    std::vector<game_drawable*> gameobjects = {&player1, &test1};
    //gameloop
    while (window.isOpen()) {
        switch (state_t) {
            case GAME: {
                drawables = drawables = {&left,&game_window, &right, &quit_gamewindow, &back_to_menu_from_gamewindow,&player1, &test1, &display_coins};
